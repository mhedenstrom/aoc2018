...
records = [
    ...
    for r in open("records.txt", "r").read().split('\n')
]


sorted_records = sorted(records, key = lambda t: t[0])

def shifts(records):
    idxs_begins_shift = [
        i for i, (_, _, action) in enumerate(sorted_records)
        if action == "begins shift"
    ]

    shift_bounds = list(zip(
        idxs_begins_shift[],
        idxs_begins_shift[1:] + [len(records) + 1]
    ))

    return [
        sorted_records[lo:hi]
        for (lo, hi) in shift_bounds
    ]
